...
local OtherServers = httpService:JSONDecode(game:HttpGet("https://games.roblox.com/v1/games/2317712696/servers/Public?sortOrder=Asc&limit=100"))
...
local function serverHop()
    if not server then
        writefile("servers.json",httpService:JSONEncode({}))
        local server = joinNew()
        teleportService:TeleportToPlaceInstance(2317712696, server)
    else
        teleportService:TeleportToPlaceInstance(2317712696, server)
    end
end
